...
function shape()
	...
	shape.prototype.update_uniforms = function( model_transform, camera_transform, projection_transform )			// Send the current matrices to the shader
		{
				...
				gl.uniform1f ( g_addrs.ambient_loc, 1 );
	...
	shape.prototype.draw = function( model_transform, camera_transform, projection_transform, texture_filename )
		{
			this.update_uniforms(model_transform, camera_transform, projection_transform);
			gl.bindBuffer( gl.ARRAY_BUFFER, this.position_buffer      );
...
function sphere( points_transform, max_subdivisions )		// Build a sphere using subdivision, starting with a tetrahedron.  Store each level of detail in separate index lists.
	{	
		...
		this.draw = function( model_transform, camera_transform, projection_transform, texture_filename, LOD ) 	
		{ 	
			this.update_uniforms(model_transform, camera_transform, projection_transform);
			gl.bindBuffer( gl.ARRAY_BUFFER, this.position_buffer      );
			gl.vertexAttribPointer( g_addrs.vPosition, 3, gl.FLOAT, false, 0, 0 );
...
function cube2( points_transform )		
	{
		shape.call(this);
		this.populate( this, mat4() );
		this.init_buffers();
...
inherit(cube2, shape);
	...
	cube2.prototype.populate = function( recipient, points_transform )
	{
			var m_strip = new rectangular_strip(); 
			for( var i = 0; i < 3; i++ )										// Build a cube by inserting six triangle strips into the lists.
				for( var j = 0; j < 2; j++ )
					m_strip.populate( recipient, 1, mult( points_transform, mult( rotate( 90, vec3( i==0, i==1, i==2 ) ), translate( j, 0, 0 ) ) ) );
	}
...
dome_lantern.prototype.populate = function( recipient )
{
		...
		vertices.push( vec3(-.25, -.5, .25) ); //0
		vertices.push( vec3(.5, .5, .5) ); //1
		vertices.push( vec3(-.5, .5, .5) ); //2
		vertices.push( vec3(.25, -.5, .25) ); //3
		
		vertices.push( vec3(-.25, -.5, -.25) ); //4
		vertices.push( vec3(.5, .5, -.5) ); //5
		vertices.push( vec3(-.5, .5, -.5) ); //6
		vertices.push( vec3(.25, -.5, -.25) ); //7

		vertices.push( vec3(-.5, .85, .5) ); //8
		vertices.push( vec3(.5, .85, .5) ); //9
		vertices.push( vec3(.5, .85, -.5) ); //10
		vertices.push( vec3(-.5, .85, -.5) ); //11
		vertices.push( vec3(0, 1.15, 0) ); //12
		
		var indices = [];
				
		indices.push(0); //front
		indices.push(3);
		indices.push(2);
		...
		normals.push( vec3(0,0,1) ); //front
		normals.push( vec3(0,0,1) );
		normals.push( vec3(0,0,1) ); //front
		normals.push( vec3(0,0,1) );
		
		normals.push( vec3(0,0,-1) ); //back
		normals.push( vec3(0,0,-1) );
		normals.push( vec3(0,0,-1) ); //back
		normals.push( vec3(0,0,-1) );

		normals.push( vec3(0,1,0) ); //top
		normals.push( vec3(0,1,0) );
		normals.push( vec3(0,1,0) ); //top
		normals.push( vec3(0,1,0) );
		
		normals.push( vec3(-1,0,0) ); //right
		normals.push( vec3(-1,0,0) );
		normals.push( vec3(-1,0,0) ); //right
		normals.push( vec3(-1,0,0) );

		normals.push( vec3(-1,0,0) ); //top left
		normals.push( vec3(-1,0,0) );
		normals.push( vec3(-1,0,0) ); //top left
		normals.push( vec3(-1,0,0) );

		normals.push( vec3(0,0,1) ); //top front
		normals.push( vec3(0,0,1) );
		normals.push( vec3(0,0,1) ); //top front
		normals.push( vec3(0,0,1) );

		normals.push( vec3(1,0,0) ); //top right
		normals.push( vec3(1,0,0) );
		normals.push( vec3(1,0,0) ); //top right
		normals.push( vec3(1,0,0) );
		
		normals.push( vec3(0,0,-1) ); //top back
		normals.push( vec3(0,0,-1) );
		normals.push( vec3(0,0,-1) ); //top back
		normals.push( vec3(0,0,-1) );
		...
		texture.push( vec2(-.25, -.5) ); //0
		texture.push( vec2(.5, .5) ); //1
		texture.push( vec2(-.5, .5) ); //2
		texture.push( vec2(.25, -.5) ); //3
		
		texture.push( vec2(-.25, -.5) ); //4
		texture.push( vec2(.5, .5) ); //5
		texture.push( vec2(-.5, .5) ); //6
		texture.push( vec2(.25, -.5) ); //7

		texture.push( vec2(-.5, .85) ); //8
		texture.push( vec2(.5, .85) ); //9
		texture.push( vec2(.5, .85) ); //10
		texture.push( vec2(-.5, .85) ); //11
		texture.push( vec2(0, 1.15) ); //12
...
box_lantern.prototype.populate = function( recipient )
{
		...
		for(var i = 0; i < 6; i++){ //front
			
			normals.push( vec3(0,0,1) ); 

		}

		for(var i = 0; i < 6; i++){ //right
			
			normals.push( vec3(1,0,0) ); 

		}

		for(var i = 0; i < 6; i++){ //back
			
			normals.push( vec3(0,0,-1) ); 

		}
		
		for(var i = 0; i < 6; i++){ //left
...
{
	recipient.vertices.push( vec3(-.5, 0, .5));
	recipient.vertices.push( vec3(.5, 0, .5));
	recipient.vertices.push( vec3(.5, 0, -.5));
	recipient.vertices.push( vec3(-.5, 0, -.5));
	recipient.vertices.push( vec3(0, 5, 0));

	recipient.indices.push(0);
	recipient.indices.push(1);
	recipient.indices.push(2);

	recipient.indices.push(0);
	recipient.indices.push(2);
	recipient.indices.push(3);

	recipient.indices.push(0);
	recipient.indices.push(1);
	recipient.indices.push(4);
	...
	recipient.texture_coords.push(1,0);
	recipient.texture_coords.push(1,1);
	recipient.texture_coords.push(1,0);
